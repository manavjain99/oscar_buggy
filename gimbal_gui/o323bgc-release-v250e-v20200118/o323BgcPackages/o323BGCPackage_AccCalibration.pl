...
sub Acc16PCalibrationCalc1PointCalibration{
  my $ax_scale = $ACC_GRAVITY;
  my $ay_scale = $ACC_GRAVITY;
  my $az_scale = $ACC_GRAVITY;
  ...
  if( $ax_zero >  0.85*$ACC_GRAVITY ){ $ax_zero -= $ACC_GRAVITY; }
  if( $ax_zero < -0.85*$ACC_GRAVITY ){ $ax_zero += $ACC_GRAVITY; }
  if( $ay_zero >  0.85*$ACC_GRAVITY ){ $ay_zero -= $ACC_GRAVITY; }
  if( $ay_zero < -0.85*$ACC_GRAVITY ){ $ay_zero += $ACC_GRAVITY; }
  if( $az_zero >  0.75*$ACC_GRAVITY ){ $az_zero -= $ACC_GRAVITY; }
  if( $az_zero < -0.75*$ACC_GRAVITY ){ $az_zero += $ACC_GRAVITY; }
...
##  if( not ConnectionIsValid() ){ Acc16PCalibrationHalt(); return 1; }
  ...
  if( $CdDataTimeCounter==0 ){ #slows down update
    if( $Acc16PCalibration_ImuNr!=1 ){ #IMU1
      ...
      $w_Acc16PCalibration->acc16pcal_temp->Text( sprintf('%.2f°', $CdData[$CdDataTemp_p]*0.01) );#$CdData[$CdDataTemp_p] );
    }else{ #IMU2
      ...
      $w_Acc16PCalibration->acc16pcal_temp->Text( sprintf('%.2f°', $CdData[$CdDataTemp2_p]*0.01) );#$CdData[$CdDataTemp_p] );
    }
  }
...
if( $CdDataTimeCounter==0 or $CdDataTimeCounter==4 or $CdDataTimeCounter==8 ){ #slows down update
  ...
  $w_Acc16PCalibration->acc16pcal_temp_av->Text( sprintf('%.2f°', $temp_av*0.01) );

  if( $Acc16PCalibration_Is1Point==1 ){
    if(( abs($ax_av) > 0.85*$ACC_GRAVITY )or
       ( abs($ay_av) > 0.85*$ACC_GRAVITY )or
       ( abs($az_av) > 0.75*$ACC_GRAVITY )   ){
      $w_Acc16PCalibration->acc16pcal_Point1_button->Enable();
